...
export function getProductValidity(product: Product, t?: any): string {
    const features = product.features as Record<string, unknown> | undefined;
...
export function getProductName(product: Product, locale?: string): string {
    const name = getLocalizedText(product.country_name, "", locale) ||
        getLocalizedText(product.country?.name, "", locale) ||
        getLocalizedText(product.region?.name, "", locale) ||
        getLocalizedText(product.name, "", locale) ||
        "eSIM Plan";

    // Fix for hardcoded "Gün" in backend-provided names when in English mode
    if (locale === 'en' && typeof name === 'string') {
        return name
            .replace(/\b(\d+)\s*Gün\b/gi, '$1 Days')
            .replace(/\b(\d+)\s*Gun\b/gi, '$1 Days')
            .replace(/\bGün\b/gi, 'Days');
    }
